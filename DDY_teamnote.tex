...
\titlespacing*{\section}
{0pt}{0pt plus 1pt minus 0pt}{-2pt plus 0pt minus 0pt}
\titlespacing*{\subsection}
{0pt}{0pt plus 1pt minus 0pt}{0pt plus 3pt minus 3pt}
...
\title{Team Note of DDY}
\author{duram21, djs100201, yunnyworld}
\date{Compiled on August 29, 2026}
...
namespace pst {
    struct node {
        int v;
        int l, r;
    } nd[45 * MAXN]; // (N+Q)logN,, 문제에 따라 여유있게.
    int root[MAXN], t = 0;
    ...
    int upd(int prev, int l, int r, int i, int v) {
        int ix = new_node(nd[prev].v + v);
        if (l == r) return ix;
        int m = (l + r) / 2;
        if (i <= m) {
            nd[ix].l = upd(nd[prev].l, l, m, i, v);
            nd[ix].r = nd[prev].r;
        }
        else {
            nd[ix].l = nd[prev].l;
            nd[ix].r = upd(nd[prev].r, m + 1, r, i, v);
        }
        return ix;
    }

    int kth(int i1, int i2, int l, int r, int k) { // k번째 작은 수
        while (l ^ r) {
...
/*
 *  pst::root[0] = pst::init(1, n); // 첫 세그트리 빈 거로 하나 만들어 두기
 *  다 닫힌 구간임.
 *
 *  pst::root[i] = pst::root[i - 1]; // 이전 거 복사
 *  pst::root[i] = pst::upd(pst::root[i], 1, n, a[i], 1); // 한 줄기 업데이트
 *  update하고 대입 필요함!
 *
 *  pst::kth(pst::root[l - 1], pst::root[r], 1, n, k) // [l, r]에 있는 수 중 k번째 작은 수
 *
 *  pst::query(pst::root[r], 1, n, l, r); // [l, r]중 답
 */
...
\subsection {HLD}
\begin{minted}{cpp}
// lazy seg 사용 코드. 기준이 노드인지 간선인지 확인 후 적절히 변형 필요.
struct Heavy_Light_Decomposition {
    ll sz[MAXN], dep[MAXN], papa[MAXN], top[MAXN], in[MAXN], out[MAXN], pv;
    vector<ll> inp[MAXN], g[MAXN];
    ...
    void update(int u, int v, int val) {
        while (top[u] ^ top[v]) {
            ...
            int st = top[u];
            seg.update(1, 1, n, in[st], in[u], val);
            u = papa[st];
        }
        // if (u == v) return; // 간선 기준일 때는 여기서 return 필요
        if (dep[u] > dep[v]) swap(u, v);
        seg.update(1, 1, n, in[u] + 1, in[v], val); // 노드 기준이면 in[u]로 해야 함.
    }
    ...
    ll query(int u, int v) {
        ...
        while (top[u] ^ top[v]) {
            if (dep[top[u]] < dep[top[v]]) swap(u, v);
            int st = top[u];
            ret = min(ret, seg.query(1, 1, n, in[st], in[u]));
            u = papa[st];
        }
        // if (u == v) return ret; // 간선 기준일땐 u == v면 반영 안 해야 함.
        if (dep[u] > dep[v]) swap(u, v);
        ret = min(ret, seg.query(1, 1, n, in[u] + 1, in[v])); // 노드 기준이면 in[u]로 해야 함.
        return ret;
    }
} hld;
\end{minted}
...
\begin{minted}{cpp}
/*
Offline range query template.

Ordinary Mo (default):
- Sort by l / BLOCK, and reverse r-order on odd blocks.
- Usually the first version to try: simple and robust.
- If Q is similar to N, BLOCK ~= sqrt(N) is standard.
- Only add / del / get_answer are problem-specific.

Hilbert Mo below uses the SAME add/del loop.
The only essential difference is the query ordering.
*/
struct Query {
    int idx, l, r;
};
...
bool mo_cmp(const Query& a, const Query& b) {
    ...
    // odd-even ordering: reduces movement of R
...
//
// int L = 0, R = -1; // current interval is empty, 0-indexed
// for (const auto& q : qs) {
//     while (L > q.l) add(--L);
//     while (R < q.r) add(++R);
//     while (L < q.l) del(L++);
...
// }
//
// Replace add, del, get_answer for each problem.
// If left/right insertion are different operations, use
// add_left / add_right / del_left / del_right separately.
\end{minted}

\subsection{mo's algorithm (hilbert order)}
\begin{minted}{cpp}
/*
Hilbert-order Mo:
- Consider each query as a point (l, r).
- Sort queries by Hilbert order instead of l/B and r.
- The query-processing while-loops are IDENTICAL to ordinary Mo.
- Often improves locality / constants when add/del is expensive.
- It is NOT asymptotically guaranteed to be faster than ordinary Mo.
- Hilbert ordering does NOT require a special add-before-del rule.

Practical choice:
1) Try ordinary Mo first.
2) If it is close to the time limit, or add/del is expensive,
   try Hilbert ordering.
*/
using ll = long long;

ll hilbert_order(int x, int y, int pw, int rot = 0) {
    if (pw == 0) return 0;
    ...
    ll sub_size = 1LL << (2 * pw - 2);
    ll ord = 1LL * seg * sub_size;
    ll sub = hilbert_order(nx, ny, pw - 1, nrot);

    if (seg == 1 || seg == 2) ord += sub;
    else ord += sub_size - sub - 1;
    return ord;
}
...
struct HilbertQuery {
    ...
    bool operator<(const HilbertQuery& q) const {
        return ord < q.ord;
    }
};
...
// Example usage:
// int pw = 0;
// while ((1LL << pw) <= N) ++pw; // 2^pw > max coordinate
//
// for (auto& q : qs)
//     q.ord = hilbert_order(q.l, q.r, pw);
...
// }
//
// Ordinary Mo vs Hilbert Mo = QUERY ORDERING only.
\end{minted}
